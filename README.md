...
Estão implementados matching factual com proveniência, otimização grounded, Truth Gates semânticos, otimização não destrutiva de experiências, vínculo entre fontes e alvos da otimização e priorização grounded de itens independentes. O projeto não inclui autenticação, banco de dados, histórico de análises, candidatura automática a vagas ou ranking externo.
