...
export function DownloadButtons({ candidate }: { candidate: Candidate }) {
  ...
  return <div><div className="flex flex-col gap-2 sm:flex-row"><button type="button" disabled={loading !== null} onClick={() => download("docx")} className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">{loading === "docx" ? "Gerando DOCX..." : "Baixar DOCX"}</button><button type="button" disabled={loading !== null} onClick={() => download("pdf")} className="rounded-xl border border-indigo-200 px-4 py-2.5 text-sm font-bold text-indigo-700 disabled:opacity-50">{loading === "pdf" ? "Gerando PDF..." : "Baixar PDF"}</button></div>{error && <p role="alert" className="mt-2 text-sm text-rose-600">{error}</p>}</div>;
}
