...
class AISemanticOptimizationTruthGate:
    ...
    def validate(
        ...
        for experience in proposal.experiences:
            ...
            for statement in experience.statements:
                payload = self._payload(experience.experience_index, statement, catalog)
                response = self._client.generate(
                    system_prompt=OPTIMIZATION_TRUTH_GATE_SYSTEM_PROMPT,
                    user_prompt=json.dumps(payload, ensure_ascii=False),
                    response_model=OptimizationStatementTruthDecision,
                )
    ...
    def _payload(
        ...
        prefix = f"experiences[{experience_index}]."
        if any(
